...
\Sem{\TypeK} &= \TypeK \\
\Sem{\LabK} &= \top \\
\Sem{\kappa_1 \to \kappa_2} &= \Pi (\alpha : \Sem{\kappa_1}). \Sem{\kappa_2} \\
\Sem{\RowK \kappa} &= \Sigma(n : \Nat). \Pi (j : \Ix  n). \Sem{\kappa} \\
\end{align*}
\end{gather*}
\begin{gather*}
\fbox{$\Sem{\tau}$} \\
...
\small
\begin{gather*}
\fbox{$\Delta \vdash \tau \leadsto \upsilon : \kappa$}
\\
\ib{\irule[\crule{foo}]{A};{B}} \\
\fbox{$\Delta \vdash M \leadsto N : \tau$}
\\
\ib{\irule[\crule{foo}]{A};{B}} \\
\fbox{${\EntJ \Delta \pi} \leadsto N$}
\\
\ib{\irule[\crule{foo}]{A};{B}}  \\
...
\begin{itemize}
\item $\Delta \vdash \tau \leadsto \upsilon : \kappa$ denotes the translation of judgment $\Delta \vdash \tau : \kappa$ to term $\Sem{\Delta} \vdash \Sem{\tau} : \Sem{\kappa}$.
\item 
\end{itemize}

Actually, I am now starting to wonder if I can give an inductive definition of the translation and then an operational semantics over translations. One could then define an erasure to both \RO and \IX{} terms. A preservation theorem would now be like a bisimulation argument---that well-typed pairs of \RO and \IX{} terms step to well-typed pairs of \RO and \IX{} terms. Doing this this way gets rid of the untyped translation and obligation to prove translational soundness.

To do this, you would really need \emph{strong} bisimulation; I doubt that \RO steps and \IX{} steps are 1-to-1. So you may have cases where an $\RO$ term ``steps'' to itself while the \IX term steps. Maybe actually this is a bad idea.

Actually... I should be able to mimic the approach of MorrisM19, which is a mix of the two.
\begin{enumerate}
\item Give a translation of types, predicates, and typing environments, e.g., $\Sem{\tau}$. (In MorrisM19, this is $(\tau)^{\bullet}$).
\item Give a typed translation $\Gamma \vdash M \leadsto N : \tau$, etc.
\item Prove soundness separately. That is, if $\Gamma \vdash M \leadsto N : \tau$ then $(\Gamma)^{\bullet} \vdash N : \tau^{\bullet}$.
\end{enumerate}

I like this approach because I am not obligated to specify the (untyped) translation of $M$ to $N$, which means mechanizing things like substitution in the untyped syntax.


Most of these notes need to be erased. These are stream of consciousness notes.
