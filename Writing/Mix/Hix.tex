...
\title{A Translation of \RO terms to \Hix}
...
\section{Specification by way of translation}
This document is intended to inform the design of $\Ix$ by consider a translation of the term, type, and kind language (in that order) of \RO to a reasonable looking dependent calculus (hereby called \Hix). 

\subsection{Translating \RO terms}

\subsubsection{STLC \& System F terms.}

\newcommand\WithType{\; \mid_{\tau} \;}
\newcommand\WithKind{\; \mid_{\kappa} \;}
\newcommand\Uone{\ensuremath{\mathcal{U}_{1}}}
\newcommand\Utwo{\ensuremath{\mathcal{U}_{2}}}

\paragraph{Arrow introduction.}
\[
\lambda x \co \Nat. x \WithType \Nat \to \Nat \WithKind \TypeK
\]

might translate to

\[
\lambda x \co \Nat. x \WithType \Pi x \co \Nat. \Nat \WithType \Uone
\]


\paragraph{Arrow elimination.}

\[
\lambda x \co \Nat \to \Nat. \lambda y \co \Nat. x \, y \WithType (\Nat \to \Nat) \to \Nat \WithKind \TypeK
\]

might translate to

\[
\lambda x \co \Nat. \lambda y \co \Nat. x \, y \WithType \Pi x \co (\Pi \_ \co \Nat. \Nat). \Pi y \co \Nat. x \cdot y \WithKind
\]
...
Consider the term
\[
\Lambda \ell : \LabK.  : \forall 
\]

\subsubsection{Records and variants.}

\subsubsection{System F$_\omega$.}

\subsection{Translating types and kinds}

\subsection{Translating predicates}

\subsection{Translating type equivalence}
