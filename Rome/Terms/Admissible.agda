...
NatPFunctorT : ∀ {ℓ ℓΔ} {Δ : KEnv ℓΔ} → Type Δ (★ (lsuc ℓ))
NatPFunctorT {ℓ} = `∀ (R[ ★ ℓ `→ ★ ℓ ]) (NatP ⇒ Functor ·[ Σ (tvar Z) ])

NatPFunctor : ∀ {ℓ ℓΓ ℓΦ} {Γ : Env Δ ℓΓ} {Φ : PEnv Δ ℓΦ} → Term Δ Φ Γ (NatPFunctorT {ℓ = ℓ})
NatPFunctor {ℓ} = `Λ _ {!   !}
