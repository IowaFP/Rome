...
open import ROmega.Types.Admissible
...
wtf : ∀ {ℓ ℓΔ} {Δ : KEnv ℓΔ} → Type Δ (★ (lsuc ℓ))
wtf {ℓ} = `∀ (L ℓ) U -- (`∀ (★ ℓ) (∅ {ℓ = ℓ}))

w : ∀ {ℓ ℓΔ ℓφ ℓΓ} {Δ : KEnv ℓΔ} {φ : PEnv Δ ℓφ}  {Γ : Env Δ ℓΓ} → Term Δ φ Γ (wtf {ℓ} {ℓΔ})
w {ℓ} {ℓΔ} {ℓΦ} {Δ = Δ} {φ = φ} = `Λ (L ℓ) u
