...
open Rμ.Kind renaming (_`→_ to _—→_)
open Rμ.Star renaming (_`→_ to _—→_)
...
open Rμ.Type renaming (_`→_ to _`—→_)

⟦_∣_∣_⟧ : (Δ : Rμ.KEnv) → Rμ.PEnv Δ → Rμ.Env Δ → Context

⟦_⟧τ-L : ∀ {Δ : Rμ.KEnv} → Rμ.Type Δ L →  Type ⟦ Δ ∣ ε ∣ ε ⟧ ★
⟦_⟧τ-★ : ∀ {Δ : Rμ.KEnv} {κ} → Rμ.Type Δ κ → Rμ.Star κ →  Type ⟦ Δ ∣ ε ∣ ε ⟧ ★
⟦_⟧τ-ρ : ∀ {Δ : Rμ.KEnv} {κ} → Rμ.Type Δ R[ κ ] →  Term ⟦ Δ ∣ ε ∣ ε ⟧ ⟦ κ ⟧κ
⟦_⟧p : ∀ {Δ : Rμ.KEnv} {κ} → Rμ.Pred Δ κ → Type ⟦ Δ ∣ ε ∣ ε ⟧ ⟦ κ ⟧σ
...
-- ⟦_⟧ : ∀ {Δ : KEnv} {Φ : Δ} {Γ : Env Δ} {τ : Type Δ ★} → 
--         Term Δ Φ Γ τ → Term ⟦ Δ ∣ Φ ∣ Γ ⟧ ⟦ τ ⟧τ
-- ⟦ M ⟧ = {!!}
