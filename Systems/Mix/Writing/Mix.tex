...
\begin{doublesyntaxarray}
  ...
  & & & &  \Sigma \alpha \co T. M \mid (\alpha \co T , M) \mid \Fst \, M \mid \Snd \, M \mid  \\
...
Let $0,1,2,...$ denote (as meta-syntax) object-level natural numbers in the intuitive fashion and let $i_{n}$ denote finite natural obtained by $n$ applications of $\FSuc$ to $\FZero$.
...
\Ixed{\pi \then \tau} &= \Pi (\alpha : \Ixed{\pi}). \Ixed{\tau} \\
\Ixed{\tau \, \upsilon} &= \Ixed{\tau} \, \Ixed{\upsilon} \\
\Ixed{\ell} &= \top  \\
\Ixed{\Sing \xi} &= \top  \\
\Ixed{\LabTy \xi \tau}&= (1, \, \lambda (i : \Ix{1}). \Ixed{\tau} )  \\
\Ixed{\Pi \rho}&=  \Pi(i : \Ix{(\Fst \, \Ixed{\rho})}). (\Snd \, \Ixed{\rho}) \, i\\
\Ixed{\Sigma \rho}&= \Sigma (i : \Ix{(\Fst \, \Ixed{\rho})}). (\Snd \, \Ixed{\rho}) \, i\\
\end{align*}
\end{gather*}
