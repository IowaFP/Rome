...
... | no p | no q = `λ (ne (weakenₖNE f · ne (` Z) {ground = fromWitness (¬Arrow→Ground p)}) {fromWitness (¬Arrow→Ground q)})
η-expand {κ₁ = κ₁ `→ κ₂} {κ₂ = κ₃ `→ κ₄} f | yes p | yes q = 
  `λ (η-expand (weakenₖNE f · (η-expand (` Z))))
η-expand {κ₁ = κ₁} {κ₂ = κ₂ `→ κ₃} f | no p | yes q =
 `λ (η-expand (weakenₖNE f · (ne (` Z) {ground = fromWitness (¬Arrow→Ground p)})))
