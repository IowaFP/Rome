...
⟦_⟧≋_ {Δ} {κ = R[ κ ]} τ (φ <$> n) = 
  (τ ≡t (⇑ (reifyKripkeNE φ) <$> ⇑NE n)) × SoundKripkeNE (⇑ (reifyKripkeNE φ)) φ
  -- (τ ≡t (⇑ (reifyKripkeNE φ) <$> ⇑NE n)) × SoundKripkeNE {!!} φ
    -- (∀ {Δ₂} (r : Renamingₖ Δ Δ₂) {v V} → 
    --   ⟦ v ⟧≋ reflect V → 
    --   ⟦ (renₖ r (⇑ (reifyKripkeNE φ)) · v) ⟧≋ (φ r V))
⟦ [] ⟧r≋ (zero , P) = ⊤
⟦ [] ⟧r≋ (suc n , P) = ⊥
⟦ x ∷ ρ ⟧r≋ (zero , P) = ⊥
...
reflect-⟦⟧≋ {κ = κ₁ `→ κ₂} {τ} {υ} e = 
    ...
    (eq-· 
        ...
        (reify-⟦⟧≋ q)) 
reflect-⟦⟧≋ {κ = R[ κ ]} e = 
  eq-trans (eq-sym eq-map-id) (eq-<$> (eq-λ (eq-sym (η-norm-≡t (` Z)))) e) , 
...
reify-⟦⟧≋ {κ = R[ κ ]} {τ} {φ <$> ρ} eq = eq .fst -- eq
...
subst-⟦⟧≋ {κ = R[ κ ]} {τ₁ = τ₁} {τ₂} q {φ <$> n} (eq , rel) = (eq-trans (eq-sym q) eq) , rel
...
↻-ren-⇑-reify-─ r (φ <$> N) V rel₁ rel₂ ev = eq-─ (eq-<$> (eq-λ {!reify-⟦⟧≋ (ren-⟦⟧≋ (liftₖ r) {⇑ (reify (φ S (` Z)))} {φ S (` Z)} ?)!}) (eq-sym (inst (↻-ren-⇑NE r N)))) {!!}
...
ren-⟦⟧≋ {κ = R[ κ ]} ρ {v} {φ <$> n} (eq , rel-v) = 
  ...
  --     (eq-trans
  --        (reify-⟦⟧≋
  --         (ren-⟦⟧≋ (liftₖ ρ) {⇑ (reify (φ S (` Z)))} {φ S (` Z)} {!!}))
  --        (inst (cong (⇑ ∘ reify) {!↻-ren!})))) 
  --     (eq-sym (inst (↻-ren-⇑NE ρ n)))) 
  --     eq-trans (renₖ-≡t ρ rel-v) ? , ?
