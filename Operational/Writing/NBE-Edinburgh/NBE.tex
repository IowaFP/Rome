...
\begin{enumerate}
  \item A normalization procedure for the directed \Rome type equivalence relation, which in turn yields a decision procedure for deciding type equivalence;
...
\section{Background}
We briefly review the literature on row typing. Row types, as introduced by \citet{wand87}, describe a type-level association of labels to types. Rows can be used dually to type \emph{records} (i.e, classes and objects) and \emph{variants} (i.e., algebraic data types). Row types were first introduced by \citet{wand87,Wand91,Remy89} to give a \emph{structural} account of object inheritance, in which records and variants are seen as equal when their structures agree. For example, the two-dimensional point record !p! !=! !{x := 1,! !y := 2}! might be assigned the record type !Pi! !{x := Int,! !y := int}! in a row-type system. Then, any function expecting a record input with the fields !{x := Int , y := int}! will be happy to accept !p!. In contrast, standard subtyping relations could be said to be \emph{nominal} in that they rely upon declared relationships between entities to convey inheritance and polymorphism. For example, two Java objects with identical fields are not immediately equatable.

Extensibility in row type systems arises from \emph{row polymorphism}, which has seen great use in e.g. the OCaml language \citep{OCaml:objTypes}. In contrast to many row type systems, \RO and \Rome use \emph{qualified types} \citep{Jones92} to constrain the instantiation of row variables in types. One immediate effect of this design choice is that \RO and \Rome are able to express rows via \emph{concatenation} rather than just by extension. The shape of predicates in \Rome are described in the next section.

Finally, Many novel applications of rows to other domains have been proposed, including first-class mixins \citep{MakholmW05}, effect typing \citep{Leijen05,Leijen14,LindleyC12,HillerstromL16} and session types \citep{LindleyM17}. Recent works have explored capabilities of \emph{scoped rows}, in which the extension of a row by another with a label overlap \emph{shadows} the overlapped label: \citet{PaszkeX23} describe the interplay of scoped rows and first-class labels, while \citet{XieCIL22} describe a system with first-class names for effect handlers. The algebraic effects literature has indeed popularized the use of rows in typing effects; The \Rose language \citep{MorrisM19}, from which \RO and \Rome extend, first sought to reconcile varying accounts of label overlap resolutions into \emph{row theories}. 

We view \Rome as a highly-expressive candidate core calculus for many applications of row types.
...
We define the equivalence relation of \cref{fig:equivalence} as an inductive, intrinsically-typed relation in Agda.
...
\subsection{Intrinsic mechanization of System \Fome and others.} Our technical development owes a huge debt to two papers in particular. We closely follow the formalization patterns and proof techniques of \citet{ChapmanKNW19}; indeed, this paper is in some sense an extension of their work from System \Fome to system \Rome. In turn, \citet{ChapmanKNW19} themselves follow closely \citet{AllaisBM13}, from whom we looked to in finding the correct normal form of inert row maps (borrowing from their formalization of lists). This paper differs from \citet{ChapmanKNW19}, but not \citet{AllaisBM13}, in that \Rome introduces additional definitional equality rules. In contrast to \citet{ChapmanKNW19}, we also reason about types modulo $\eta$-equivalence of functions and also expansion of rows to inert maps, which made many proof definitions harder to define.

\subsection{Mechanized Ur/Web.} A comparison must be made between \Rome and Featherweight Ur \citep{Chlipala10}, each of which have type-level rows and mapping operators. However, Ur focuses mainly on type-level records and does not witness the duality between $\Pi$ and $\Sigma$ types. Further, \citet{Chlipala10} give an elaborative semantics, translating Featherweight Ur to the Calculus of Inductive Constructions, in order to prove metatheory. It is unclear if a procedure has been mechanized to normalize Featherweight Ur programs according to its own definitional equality rules: the authors conjecture that "refactoring the definitional equality rules of [Featherweight Ur] so that, when applied only left-to-right, [will form] a rewrite [that is] terminating and confluent." To their credit, Featherweight Ur is more concerned with practical higher-order unification and inference implementation details, of which \Rome can stake no parallel claim.
