...
This section describes the Agda formalization of the metatheory summarized in \cref{sec:metatheory-summary}, including proofs and proof outlines where space permits.
...
Soundness states the converse of completeness: if two types normalize to the same form, then they are equivalent. 
...
\Ni In practice, it is easier to instead prove that each type is equivalent to its embedded normalization: 

\begin{agda}
soundness : forall (tau : Type Delta kappa) -> tau .=.t Up (Down tau)
\end{agda}

\Ni And it is straightforward to show that the latter implies the former:
