...
data EvProgress {π : NormalPred Δ R[ κ ]} (M : NormalEnt Γ π) : Set where
  Done : 
         EvValue Γ π M → 
         ----------
         EvProgress M
...
evProgress : ∀ {π : NormalPred ∅ R[ κ ]} (N : NormalEnt ∅ π) → EvProgress N
evProgress (n-incl i₁) = Done (n-incl i₁)
evProgress (n-plus i₁ i₂ i₃) = Done (n-plus i₁ i₂ i₃)
evProgress n@n-refl with norm-≲ n 
... | xs , oxs , ys , oys , refl , refl = StepsTo (n-incl (λ x i → i)) via δ-refl xs oxs
evProgress (_n-⨾_ M N) with evProgress M | evProgress N
...
evProgress (n-complL-inert N) with norm-≲ N 
evProgress (n-complL-inert {nsr = ()} N) | xs , _ , ys , _ , refl , refl
evProgress (n-complL N) with evProgress N 
... | Done (n-incl {xs = xs} {ys = ys} {oxs = oxs} {oys} i) = 
...
progress (M₁ · M₂) | Done (V-▿ F G vF vG) | StepsTo M₂' via M₂—→M₂' = StepsTo (F ▿ G) _ · M₂' via ξ-·2 (V-▿ F G vF vG) M₂—→M₂'
progress (M₁ · M₂) | Done (V-▿ {e = e} F G vF vG) | Done (V-Σ {τ = τ} l {M} vM i) with evProgress e 
...
progress (M₁ · M₂) | Done (V-ana ρ φ τ eq₁ eq₂ M vM) | StepsTo M₂' via M₂—→M₂' = StepsTo ana ρ φ τ eq₁ eq₂ M · M₂' via ξ-·2 (V-ana ρ φ τ eq₁ eq₂ M vM) M₂—→M₂' 
...
progress (prj M n) with progress M | evProgress n | norm-≲ n
...
progress ((M₁ ⊹ M₂) n) with progress M₁ | progress M₂ | evProgress n 
...
progress (inj M n) with progress M | evProgress n 
